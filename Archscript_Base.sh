...
pacman -S xorg-server xorg-xinit xorg-xkill xorg-xsetroot xorg-xbacklight xorg-xprop xorg-xrandr xorg-xinput \
    bspwm sxhkd dunst pavucontrol libnotify acpi acpi_call lxappearance papirus-icon-theme arc-gtk-theme rofi nitrogen \
    zsh zsh-syntax-highlighting zsh-autosuggestions kitty ranger mpd playerctl mpc ncmpcpp \
    neofetch lolcat htop bashtop keepassxc neovim \
    hunspell hunspell-en_us hyphen hyphen-en libmythes mythes-en feh firefox starship dust bat exa \
    xfce4-clipman-plugin rsync maim xdotool noto-fonts noto-fonts-emoji \
    ttf-joypixels ttf-font-awesome sxiv mpv imagemagick fzf gzip p7zip libzip zip unzip yt-dlp xclip \
    dhcpcd networkmanager network-manager-applet pamixer paprefs pulseaudio pulseaudio-alsa pulsemixer pavucontrol alsa-utils alsa-firmware sof-firmware man-db  \
    zathura zathura-pdf-mupdf xdg-user-dirs xdg-utils unclutter lightdm lightdm-gtk-greeter \
	dialog wpa_supplicant mtools linux-headers gvfs gvfs-smb nfs-utils inetutils dnsutils tlp tlp-rdw firewalld acpid 
